...
void HandleMurderOfCivilian(const SOLDIERTYPE* const pSoldier)
{
	...
	//HB: but penalize dead militia
	if ( ( ( pSoldier->bTeam != CIV_TEAM )  && ( pSoldier->bTeam != MILITIA_TEAM ) ) || ( pSoldier->ubBodyType == CROW ) )
	{
		return;
	}
	...
	//HB: or the player trained the killed one
	if( ( bKillerTeam == OUR_TEAM ) || ( pSoldier->bTeam == MILITIA_TEAM ) )
	{
...
void HandleGlobalLoyaltyEvent( UINT8 ubEventType, INT16 sSectorX, INT16 sSectorY, INT8 bSectorZ)
{
	...
	switch (ubEventType)
	{
		...
		case GLOBAL_LOYALTY_NATIVE_KILLED:
			...
			//HB: penalize militia death a bit more
			iLoyaltyChange = -7 * GAIN_PTS_PER_LOYALTY_PT;
			//iLoyaltyChange = -50;
			break;
