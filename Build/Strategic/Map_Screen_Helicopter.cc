...
BOOLEAN WillAirRaidBeStopped( INT16 sSectorX, INT16 sSectorY )
{
	...
	if (!(StrategicMap[SECTOR_INFO_TO_STRATEGIC_INDEX( SEC_N4 )].fEnemyControlled)
			|| !(StrategicMap[SECTOR_INFO_TO_STRATEGIC_INDEX( SEC_N3 )].fEnemyControlled))
	{
		return( TRUE ); //HB: no airport, no air raid
	}


	if ( NightTime() )
	{
		return( TRUE ); //HB: air raids only at daytime
	}
