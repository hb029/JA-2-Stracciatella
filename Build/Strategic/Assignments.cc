...
#define MINUTES_FOR_ASSIGNMENT_TO_COUNT	60
...
static BOOLEAN CanCharacterPatient(const SOLDIERTYPE* const s)
{
	...
	return
		s->bLife > 0 &&
		(s->bLife < (DOCTOR_RATING_CAP * s->bLifeMax)/100) &&
...
static void UpdatePatientsWhoAreDoneHealing()
{
	FOR_EACH_IN_TEAM(i, OUR_TEAM)
	{
		...
		if (s.bLife < (DOCTOR_RATING_CAP * s.bLifeMax)/100) continue;
...
static BOOLEAN CanSoldierBeHealedByDoctor(SOLDIERTYPE const* const patient, SOLDIERTYPE const* const doctor, BOOLEAN const fThisHour, BOOLEAN const fSkipKitCheck, BOOLEAN const fSkipSkillCheck)
{
	...
	if (patient->bLife >= (DOCTOR_RATING_CAP*patient->bLifeMax)/100)	 		  return FALSE;
...
//	INT8 bOldPatientLife = pPatient -> bLife;
	...
	if (pPatient -> bLife < (DOCTOR_RATING_CAP * pPatient -> bLifeMax)/100)
	{
		bPointsToUse = ( (DOCTOR_RATING_CAP * pPatient -> bLifeMax)/100 - pPatient -> bLife );
		...
		for (bPocket = HANDPOS; bPocket <= SMALLPOCK8POS; bPocket++)
		{
			...
			if (IsMedicalKitItem(&o))
			{
				...
				if ( ( bPointsToUse <= 0 ) || ( pPatient -> bLife >= (DOCTOR_RATING_CAP * pPatient -> bLifeMax)/100 ) )
				{
					break;
				}
			}
	...
	if( pPatient->bLife >= __min(DOCTOR_RATING_CAP, pPatient->bLifeMax) )
	{
...
*/
	...
	if ( pPatient -> bLife < (HOSPITAL_RATING_CAP * pPatient -> bLifeMax)/100 )
	{
		bPointsToUse = ( (HOSPITAL_RATING_CAP * pPatient -> bLifeMax)/100 - pPatient -> bLife );
...
// No point in allowing SAM site repair any more.  Jan/13/99.  ARM //HB: ressurect this feature
...
static void DoActualRepair(SOLDIERTYPE* pSoldier, UINT16 usItem, INT8* pbStatus, UINT8* pubRepairPtsLeft)
{
	...
	sRepairCostAdj = REAPAIR_RATING_CAP - ( 10 * Item[ usItem ].bRepairEase );
	...
	else	// not enough, partial fix only, if any at all
	{
		...
		usPtsFixed = ( *pubRepairPtsLeft * REAPAIR_RATING_CAP ) / sRepairCostAdj;
		...
		if (usPtsFixed > 0)
		{
			*pbStatus += usPtsFixed;
			...
			if ( *pbStatus > REAPAIR_RATING_CAP )
			{
				*pbStatus = REAPAIR_RATING_CAP;
			}
		}
...
{
	return status < REAPAIR_RATING_CAP && Item[item_id].fFlags & ITEM_REPAIRABLE;
}
...
void MakeSoldiersTacticalAnimationReflectAssignment(SOLDIERTYPE* const s)
{
	...
	switch (s->bAssignment)
	{
		...
		default:
			...
			{
				//HB: exploit for standing up in battle without ap
...
void DetermineWhichAssignmentMenusCanBeShown(void)
{
	...
	if (!fShowAssignmentMenu)// || fCharacterNoLongerValid)//HB: fix this ctd better
	{
...
static void RemoveMercMenuBtnCallback(MOUSE_REGION* pRegion, INT32 iReason)
{
	...
	//pSoldier = GetSelectedAssignSoldier( FALSE );
	pSoldier = GetSelectedInfoChar(); //HB: do not ask why this fixes the ctd!
...
static BOOLEAN CanCharacterRepairRobot(SOLDIERTYPE const* const pSoldier)
{
	...
	//HB: repair is limited
	if ( pRobot -> bLife >= REAPAIR_RATING_CAP || pRobot -> bLife == pRobot -> bLifeMax )
	{
		return( FALSE );
	}
...
static UINT8 RepairRobot(SOLDIERTYPE* pRobot, UINT8 ubRepairPts, BOOLEAN* pfNothingLeftToRepair)
{
	...
	if( pRobot -> bLife + ubRepairPts >= REAPAIR_RATING_CAP )
	{
		ubPointsUsed = ( REAPAIR_RATING_CAP - pRobot -> bLife );
		pRobot -> bLife = REAPAIR_RATING_CAP;
	}
	else
	{
	...
	if ( pRobot->bLife >= REAPAIR_RATING_CAP )
	{
		*pfNothingLeftToRepair = TRUE;
	}
	else
...
//* No point in allowing SAM site repair any more.  Jan/13/99.  ARM//HB: feature on again!
BOOLEAN CanSoldierRepairSAM( SOLDIERTYPE *pSoldier, INT8 bRepairPoints)
{
	...
	FOR_EACH_IN_TEAM(pSoldier, OUR_TEAM)
	{
		switch (pSoldier->bAssignment)
		{
			...
			case TRAIN_BY_OTHER:
				fNothingToDo = !CanCharacterTrainStat(pSoldier, pSoldier->bTrainStat, TRUE, FALSE);
				break;
				//HB: without trainer as beneficial and exhausting as self-training
				//fNothingToDo = !CanCharacterTrainStat(pSoldier, pSoldier->bTrainStat, TRUE, FALSE) ||
