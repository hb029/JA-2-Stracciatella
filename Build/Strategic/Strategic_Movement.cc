...
{
	return FALSE; //HB: this feature is off
...
static void PrepareForPreBattleInterface(GROUP* pPlayerDialogGroup, GROUP* pInitiatingBattleGroup)
{
	...
	{
		if( ( pPlayerDialogGroup->uiFlags & GROUPFLAG_JUST_RETREATED_FROM_BATTLE )
				|| !pInitiatingBattleGroup->fPlayer) //HB: never retreat from defense
		{
			gfCantRetreatInPBI = TRUE;
		}
...
void GroupArrivedAtSector(GROUP& g, BOOLEAN const check_for_battle, BOOLEAN const never_left)
{
	...
	{
		if( !g.fPlayer && GetTownIdForSector(SECTOR(x, y))
		&& FindPlayerMovementGroupInSector(x, y)
		&& g.ubSectorIDOfLastReassignment != SECTOR( x, y )
		&& PreChance(33))
		{
			//HB: scare off defenders
			AIR_RAID_DEFINITION	AirRaidDef;
			AirRaidDef.sSectorX		= x;
			AirRaidDef.sSectorY		= y;
			AirRaidDef.sSectorZ		= 0;
			AirRaidDef.bIntensity 	= 3 + Random(2);
			AirRaidDef.ubNumMinsFromCurrentTime	= 1;

			g.uiArrivalTime += AirRaidDef.bIntensity;

			ScheduleAirRaid( &AirRaidDef );
			g.ubSectorIDOfLastReassignment = (UINT8)SECTOR( x, y );
