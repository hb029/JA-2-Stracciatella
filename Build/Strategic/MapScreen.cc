...
static void HandleModNone(UINT32 const key, MapEvent& new_event)
{
	switch (key)
	{
		...
		//HB: drop all button for equipment from scratch
		case 'd': if (!fShowInventoryFlag && !fShowMapInventoryPool && !(gTacticalStatus.fEnemyInSector))
			{
				DropAllItems();
			}
		break;
...
static void DetermineIfContractMenuCanBeShown(void)
{
	if (!fShowContractMenu)
	{
		...
		// make sure, absolutely sure we want to hide this box
