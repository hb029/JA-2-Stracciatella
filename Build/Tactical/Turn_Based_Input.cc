...
void GetPolledKeyboardInput(UIEventKind* puiNewEvent)
{
	...
	//static BOOLEAN	fAltDown = FALSE;//HB: off
	...
	// CHECK FOR ALT
	//HB: the animations are buggy, so this is off
	/*
	switch( gCurrentUIMode )
	{
			case MOVE_MODE:
...
static void HandleModNone(UINT32 const key, UIEventKind* const new_event)
{
	switch (key)
	{
		...
		//case 'a': BeginAutoBandage(); break; //HB: this feature is off for time warp
	...
	switch (key)
	{
		...
		case 'a':
			if (CHEATER_CHEAT_LEVEL())
			{
				...
				AirRaidDef.sSectorX		= gWorldSectorX;
				AirRaidDef.sSectorY		= gWorldSectorY;
				AirRaidDef.sSectorZ		= 0;
				AirRaidDef.bIntensity 	= 2;
				AirRaidDef.uiFlags		= AIR_RAID_BEGINNING_GAME;
				AirRaidDef.ubNumMinsFromCurrentTime	= 0;
