				...
				switch (pSoldier->ubProfile)
				{
					case SKYRIDER:
						if (PythSpacesAway(pSoldier->sGridNo, 8842) < 11)
						{
							...
							//HB: make sure skyrider does not spawn at the edge of the map
							gMercProfiles[ SKYRIDER ].ubStrategicInsertionCode = INSERTION_CODE_GRIDNO;
							gMercProfiles[ SKYRIDER ].usStrategicInsertionData = WORLD_ROWS / 2 * WORLD_COLS + WORLD_COLS / 2;
						}
						break;
...
BOOLEAN CheckForEndOfBattle( BOOLEAN fAnEnemyRetreated )
{
	...
	if ( gfInAirRaid )
	{
		//HB: resolve the air raid situation first
		return( FALSE );
	}
...
//	}
	...
	if ( gTacticalStatus.uiFlags & CHECK_SIGHT_AT_END_OF_ATTACK )
	{
		...
		//HB: with time on, hidden enemies would move without sight-check!
		UnPauseGame();
	}
...
static void EndBattleWithUnconsciousGuysCallback(MessageBoxReturnValue const bExitValue)
{
	...
	//HB: otherwise some buttons remain
