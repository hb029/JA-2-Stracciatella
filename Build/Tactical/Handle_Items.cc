...
INT32 InternalAddItemToPool(INT16* const psGridNo, OBJECTTYPE* const pObject, Visibility bVisible, UINT8 ubLevel, UINT16 usFlags, INT8 bRenderZHeightAboveLevel)
{
	...
	*psGridNo = sNewGridNo;
	...
	// If bvisible is true, render makered world
...
BOOLEAN NearbyGroundSeemsWrong(SOLDIERTYPE* const s, const INT16 sGridNo, const BOOLEAN fCheckAroundGridno, INT16* const psProblemGridNo)
{
	BOOLEAN fMining;
	UINT8   ubDetectLevel;
	//HB: detector must be in hand while crouching or lying
	if (FindObj(s, METALDETECTOR) == HANDPOS && ( gAnimControl[ s->usAnimState ].ubHeight == ANIM_CROUCH || gAnimControl[ s->usAnimState ].ubHeight == ANIM_PRONE) )
	{
		fMining       = TRUE;
		ubDetectLevel = 0;
	}
