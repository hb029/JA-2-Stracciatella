...
void HandleNPCDoAction( UINT8 ubTargetNPC, UINT16 usActionCode, UINT8 ubQuoteNum )
{
	...
	else
	{
		switch( usActionCode )
		{
			...
			{
				// Delete menu, give item to miguel
				DeleteTalkingMenu( );
			...
			case NPC_ACTION_DOCTOR_ESCORT_PATIENTS:
			{
				...
				if ( pSoldier2 )
				{
					...
					FOR_EACH_IN_TEAM(i, gbPlayerNum)
					{
						...
						// Are we in this sector, On the current squad?
						if (s.bInSector &&
								( (0 < s.bLife && s.bLife < (HOSPITAL_RATING_CAP * s.bLifeMax)/100) || (s.bBleeding > 0) ) &&
...
UINT32 CalcPatientMedicalCost(const SOLDIERTYPE* const pSoldier)
{
	...
	uiCost = 10 * ( __max( (HOSPITAL_RATING_CAP * pSoldier->bLifeMax)/100 - pSoldier->bLife, 0 ) );
...
UINT32 CalcMedicalCost( UINT8 ubId )
{
	...
	CFOR_EACH_IN_TEAM(s, gbPlayerNum)
	{
		if (s->bInSector
				&& 0 < s->bLife
				&& s->bLife < (HOSPITAL_RATING_CAP * s->bLifeMax)/100
				&& s->bAssignment != ASSIGNMENT_HOSPITAL
