...
static void AddItemToArmsDealerInventory(ArmsDealerID const ubArmsDealer, UINT16 const usItemIndex, SPECIAL_ITEM_INFO* const pSpclItemInfo, UINT8 ubHowMany)
{
	...
	//HB: Hack regen boosters are Scifi
	if ( usItemIndex == REGEN_BOOSTER && !gGameOptions.fSciFi)
	{
		ubHowMany = 0;
	}
...
UINT32 CalculateObjectItemRepairCost(ArmsDealerID const ubArmsDealer, OBJECTTYPE const* const pItemObject)
{
	...
	for ( ubCnt = 0; ubCnt < MAX_ATTACHMENTS; ubCnt++ )
	{
		...
		{
			// if damaged and repairable
			if ( ( pItemObject->bAttachStatus[ ubCnt ] < SHOP_REAPAIR_RATING_CAP ) && CanDealerRepairItem( ubArmsDealer, pItemObject->usAttachItem[ ubCnt ] ) )
			{
...
//	UINT32	uiDifFrom10=0;
	...
	sRepairCostAdj = SHOP_REAPAIR_RATING_CAP - ( 10 * Item[ usItemIndex ].bRepairEase );
	...
	uiRepairCost = (UINT32)( uiItemCost * ( sRepairCostAdj * (SHOP_REAPAIR_RATING_CAP - bItemCondition) / ((FLOAT)SHOP_REAPAIR_RATING_CAP * SHOP_REAPAIR_RATING_CAP) ));
