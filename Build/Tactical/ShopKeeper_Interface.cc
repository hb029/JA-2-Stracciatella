...
static BOOLEAN RepairIsDone(UINT16 usItemIndex, UINT8 ubElement)
{
	...
	if ( CanDealerRepairItem( gbSelectedArmsDealerID, RepairItem.ItemObject.usItem ) )
	{
		...
		RepairItem.ItemObject.bStatus[ 0 ] = __max(SHOP_REAPAIR_RATING_CAP, RepairItem.ItemObject.bStatus[ 0 ]);
	}
...
*/
			...
			{
				// fix it up
				RepairItem.ItemObject.bAttachStatus[ ubCnt ] = __max(SHOP_REAPAIR_RATING_CAP, RepairItem.ItemObject.bAttachStatus[ ubCnt ]);
			}
		}
	}
...
static UINT32 EvaluateInvSlot(INVENTORY_IN_SLOT* pInvSlot)
{
	...
	{
		//HB: alcohol gifts were not applied to soldier
