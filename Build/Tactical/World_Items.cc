...
void LoadWorldItemsFromMap(HWFILE const f)
{
	...
	{	/* Add all of the items to the world indirectly through AddItemToPool, but
		...
		if (!gfEditMode)
		{
			// Check for matching item existence modes and only add if there is a match
			...
			if (!gGameOptions.fGunNut)
			{
				...
				if (item.usItemClass == IC_GUN)
				{
					...
					if (replacement != NOTHING)
					{
						...
						//HB: do not eject the wrong ammo clip!
						o.usGunAmmoItem = StandardGunListAmmoReplacement(o.usGunAmmoItem);
					}
				}
