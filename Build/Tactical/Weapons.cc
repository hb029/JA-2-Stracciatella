...
WEAPONTYPE const Weapon[] =
{
  ...
  /* rocket rifle      */         RIFLE(AMMOROCKET, 20, 54, 2, 10,         62,  5, 600, 80, 10, S_SMALL_ROCKET_LAUNCHER              ),
  ...
  /* auto rocket rifle */       ASRIFLE(AMMOROCKET, 20, 54, 2, 12, 5, 10,  97,  5, 600, 80, 10, S_SMALL_ROCKET_LAUNCHER, S_BURSTTYPE1), //HB: rockets were underpowered
...
EXPLOSIVETYPE const Explosive[] =
{
	...
	{		EXPLOSV_TEARGAS,	    0,			20,				4,				0,				1,					TARGAS_EXP		/* tear gas grenade   */},
	{ 	EXPLOSV_MUSTGAS,	    15,			40,				4,				0,				1,					MUSTARD_EXP		/* mustard gas grenade*/},
	{ 	EXPLOSV_NORMAL,		    15,			7,				3,				15,				0,					BLAST_1				/* mini hand grenade  */},
	{ 	EXPLOSV_NORMAL,		    25,			10,				4,				25,				0,					BLAST_1				/* reg hand grenade   */},
	...
	{		EXPLOSV_TEARGAS,	    0,			20,				3,				0,				1,					TARGAS_EXP,		/* GL tear gas grenade*/},
	{		EXPLOSV_STUN,			    1,			50,				4,				0,				0,					STUN_BLAST,	  /* GL stun grenade		*/},
	{		EXPLOSV_SMOKE,		    0,			0,				3,				0,				1,					SMOKE_EXP,		/* GL smoke grenade		*/},

	{		EXPLOSV_SMOKE,		    0,			0,				4,				0,				1,					SMOKE_EXP,		/* smoke grenade			*/},
...
void UseHandToHand(SOLDIERTYPE* const pSoldier, INT16 const sTargetGridNo, BOOLEAN const fStealing)
{
	...
	//HB: this commented did not work
	if (!fStealing)
	{
 		sAPCost = CalcTotalAPsToAttack( pSoldier, sTargetGridNo, FALSE, pSoldier->bAimTime );
...
UINT32 CalcChanceToHitGun(SOLDIERTYPE *pSoldier, UINT16 sGridNo, UINT8 ubAimTime, UINT8 ubAimPos )
{
  ...
  usInHand = pSoldier->usAttackingWeapon;
	...
	if (pTarget != NULL)
	{
		...
		iChance -= iPenalty;
		*/
		// HB: relative to aiming now
		if (ubAimTime > 0)
		{
			ubAimTime = __max( ubAimTime - ((pTarget->bTilesMoved * 3) / 20), 1 );
		}
	...
	if ( iSightRange > (sDistVis * CELL_X_SIZE) )
	{
		...
		// shooting beyond max normal vision... disabled
		return 0;
	}
...
INT32 CalcMaxTossRange(const SOLDIERTYPE* pSoldier, UINT16 usItem, BOOLEAN fArmed)
{
	...
	else
	{
		...
		else if ( Item[ usItem ].usItemClass == IC_GRENADE )
		{
			...
			//HB: there are heavy grenades which disappeared in the minimum(., 4)
			iRange = 2 + ( iThrowingStrength / __min( ( 3 + (Item[usItem].ubWeight) / 3 ), 5 ) );
		}
		else
		{	// not as aerodynamic!
