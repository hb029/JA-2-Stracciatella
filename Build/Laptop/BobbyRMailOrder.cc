...
//		if( BobbyRayPurchases[ i ].ubNumberPurchased )
	...
	//HB: other than 0 shipping cost caused errors when cost had 5 as last digit
	DisplayShippingCosts( fCalledFromOrderPage, 0, usGridX, usGridY, iOrderNum );
}
...
//		iTotal = giGrandTotal;
	...
	else
	{
		...
		//HB: full kg count
		iShippingCost = (INT32)( ( ( gpNewBobbyrShipments[ iOrderNum ].uiPackageWeight + 5 ) / 10 ) * usStandardCost );
	}
...
static void DisplayShippingLocationCity(void)
{
	...
	if( gbSelectedCity != -1 )
	{
		SPrintMoney(sTemp, (INT32) ( BobbyROrderLocations[gbSelectedCity].usOverNightExpress / GetWeightBasedOnMetricOption(1) ));
	}
	...
	if( gbSelectedCity != -1 )
	{
		SPrintMoney(sTemp, (INT32) ( BobbyROrderLocations[gbSelectedCity].us2DaysService / GetWeightBasedOnMetricOption(1) ));
	}
	...
	if( gbSelectedCity != -1 )
	{
		SPrintMoney(sTemp, (INT32) ( BobbyROrderLocations[gbSelectedCity].usStandardService / GetWeightBasedOnMetricOption(1)));
	}
...
///	usStandardCost = (UINT16) GetWeightBasedOnMetricOption( usStandardCost );
	...
	uiTotalCost = (UINT32)( ( (uiTotalWeight + 5 ) / 10) * usStandardCost );
