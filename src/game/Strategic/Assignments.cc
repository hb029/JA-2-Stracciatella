...
BOOLEAN IsTheSAMSiteInSectorRepairable( INT16 sSectorX, INT16 sSectorY, INT16 sSectorZ )
{
	...
	for (const SamSiteModel* samSite : GCM->getSamSites())
	{
		if (samSite->sectorId == SECTOR( sSectorX, sSectorY ))
		{
...
BOOLEAN SoldierInSameSectorAsSAM( const SOLDIERTYPE *pSoldier )
{
	...
	{
		if(samSite->sectorId == SECTOR(pSoldier->sSectorX, pSoldier->sSectorY))
		{
			return( TRUE );
		}
	}
