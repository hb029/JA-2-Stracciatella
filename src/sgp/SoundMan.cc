...
static SAMPLETAG* SoundLoadBuffer(SDL_AudioFormat format, UINT8 channels, int freq, UINT8* buffer, UINT32 size)
{
	...
	if (cvt.needed)
	{
		...
		if (SDL_ConvertAudio(&cvt) == -1) {
			SLOGD(DEBUG_TAG_SOUND, "SoundLoadBuffer Error: error converting audio - %s", SDL_GetError());
			MemFree(cvt.buf);
			return NULL;
		}
...
static SAMPLETAG* SoundLoadDisk(const char* pFilename)
{
	...
	if(pFilename[0] == '\0') {
		SLOGD(DEBUG_TAG_SOUND, "SoundLoadDisk Error: pFilename is an empty string.");
		return NULL;
	}
	...
	{
		SLOGD(DEBUG_TAG_SOUND, "SoundLoadDisk Error: %s", err.what());
		return NULL;
	}
